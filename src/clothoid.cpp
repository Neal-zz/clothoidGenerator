...
void Clothoid::getPath(State a, State b) {
    ...
    double beta = atan2((b.y - a.y), (b.x - a.x));  // 返回 (-pi,pi]
    // 可由两条对称的 clothoids 达到目标
    if ((signum(beta - b.theta) * signum(beta - a.theta) < 0) && (fabs(b.theta - a.theta) < PI)) {
        cout << "Symmetric:\n" << endl;

        start = a;
        end = b;
        /* 我们希望最终路径 = 一条线段 + 两条对称的 clothoids；
        此处首先寻找这条线段，并将 起点/终点 移动到该线段末；
        使得起点与终点可以由两条对称的 clothoids 连接*/
        // 首先寻找两条线段的交点（sharp turn point）
        double sharp_x = ((a.y-tan(a.theta)*a.x) - (b.y-tan(b.theta)*b.x)) /
            (tan(b.theta)-tan(a.theta));
        double sharp_y = a.y + tan(a.theta)*(sharp_x-a.x);
        std::cout << "sharp turn point: " << sharp_x << ", " << sharp_y << std::endl;
        // 比较 a b 点与 sharp turn point 之间的距离，较远的点需要被移动。
        double lineLength = sqrt((a.x-sharp_x)*(a.x-sharp_x)+(a.y-sharp_y)*(a.y-sharp_y)) -
            sqrt((b.x-sharp_x)*(b.x-sharp_x)+(b.y-sharp_y)*(b.y-sharp_y));
        if (lineLength < 0) { 
            end.x = b.x - fabs(lineLength)*cos(b.theta);
            end.y = b.y - fabs(lineLength)*sin(b.theta);
        }
        else {
            start.x = a.x + fabs(lineLength)*cos(a.theta);
            start.y = a.y + fabs(lineLength)*sin(a.theta);
        }

        double alpha = inRange(end.theta - start.theta) / 2;  // 转角的一半
        double D = calcD(fabs(alpha));
        // sigma 对应我论文中的 alpha0
